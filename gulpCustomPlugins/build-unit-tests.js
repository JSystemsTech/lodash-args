...
var buildUnitTests = function(type, helpersRequireSource, wrapperTestDescription) {
   return through.obj(function(file, encoding, callback) {
      fs.readFile('./src/lodash-args-spec.js', function(err, data) {
         if (err) {
         ...
         if (file.isBuffer()) {
            ...
            if (helpersRequireSource) {
               var imports = tests[0].replace('./lodash-args', helpersRequireSource);
               ...
               generatedTests = mainUnitTests.replace(/lodashArgHelpers./g, replaceStrings[type]);
